...
function renderPastSim(mode, histData, rule){
  ...
  const bs = (!Array.isArray(histData) && histData?.binSize && isFinite(histData.binSize)) ? histData.binSize : 1;
  const minBin = (!Array.isArray(histData)) ? (histData?.min ?? 0) : 0;
  ...
  const total = histData?.total || counts.reduce((a,b)=>a+b,0) || 1;
  canvas._simMeta = { hist: histData, threshold: thr, total };
...
function ensurePastSimHover(canvas){
  ...
  canvas.addEventListener("mousemove", (ev)=>{
    ...
    if (!meta || !meta.hist) return hidePastSimTip();
    const hist = meta.hist;
    const counts = Array.isArray(hist) ? hist : hist?.counts;
    if (!counts || !counts.length) return hidePastSimTip();
    ...
    const bs = (!Array.isArray(hist) && hist.binSize && isFinite(hist.binSize)) ? hist.binSize : 1;
    const startSeat = (!Array.isArray(hist) ? (hist.min ?? 0) : 0) + idx * bs;
