...
const SEAT_RULES = {
  2024: {
    ...
    senate:    { total:100, majorityLine:51,  baseR:39, baseD:28 },
    governor:  { total:50,  majorityLine:26,  baseR:19, baseD:20 },
    house:     { total:435, majorityLine:218, baseR:0,  baseD:0  }
  }
};

/* ---------- Electoral votes (2024 apportionment) ---------- */
...
/* ---------- Contested races per year ---------- */
const RACES = {
  2024: {
    senate: new Set(["AZ","CA","CT","DE","FL","HI","IN","ME","MD","MA","MI","MN","MO","MT","NE","NV","NJ","NM","NY","ND","OH","PA","RI","TN","TX","UT","VA","VT","WA","WV","WI","WY","MS"]),
...
async function renderPastYear(year){
  ...
  for (const mode of PAST_MODES){
    ...
    // Filter to contested races only for senate/governor
    const raceFilter = RACES[year]?.[mode];
    const states = raceFilter ? allStates.filter(st => raceFilter.has(st)) : allStates;

    let demWins = baseD;
    for (const st of states){
      const model = getStateModelPast(year, mode, st);
      if (!model) continue;
      // President: weight by electoral votes. Others: 1 seat each.
      const weight = (mode === "president") ? (EV[st] || 0) : 1;
      demWins += model.winProb.pD * weight;
    }
...
async function renderPastMap(year, mode, d, rule){
  ...
  gRoot.selectAll("path")
    ...
    .attr("class", dd => {
      ...
      if (!st || !d?.ratios[st]) return "state";
      if (raceFilter && !raceFilter.has(st)) return "state";
      return "state active";
    })
    ...
    .attr("fill", dd => {
      ...
      if (raceFilter && !raceFilter.has(st)) return "#e5e7eb";
      const model = getStateModelPast(year, mode, st);
    ...
    .on("mouseenter", (event, dd) => {
      ...
      if (raceFilter && !raceFilter.has(st)) return;
