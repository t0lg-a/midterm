...
function renderPastSim(mode, histData, rule){
  ...
  const binSize = (!Array.isArray(histData) && histData?.binSize) ? histData.binSize : 1;

  if (counts && counts.length){
    // Compute actual seat range of data
    const maxBin = minBin + (counts.length - 1) * binSize;
    // Pad by ~15% on each side so bars don't slam against edges
    const range = maxBin - minBin + binSize;
    const pad = Math.max(range * 0.15, binSize * 2);
    const viewMin = Math.max(0, minBin - pad);
    const viewMax = Math.min(total, maxBin + binSize + pad);
    const viewRange = viewMax - viewMin;

    const toX = (seat) => ((seat - viewMin) / viewRange) * w;
    const barW = Math.max((binSize / viewRange) * w - 0.5, 1);
    ...
    for (let i = 0; i < counts.length; i++){
      ...
      const seatIdx = minBin + i * binSize;
      const barH2 = (counts[i] / maxC) * h * 0.9;
      const bx = toX(seatIdx);
      ctx.fillStyle = (seatIdx + binSize / 2) < maj ? "rgba(37,99,235,0.5)" : "rgba(220,38,38,0.5)";
      ctx.fillRect(bx, h - barH2, barW, barH2);
    }
    ...
    // No data — just draw majority line in proportional position
