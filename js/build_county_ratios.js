...
async function main() {
  ...
  let natDem = 0, natGop = 0, natTotal = 0;
  for (const r of rows) {
    natDem += r.votesDem;
    natGop += r.votesGop;
    natTotal += r.totalVotes;
  }
  const natDemPct = (natDem / natTotal) * 100;
  const natGopPct = (natGop / natTotal) * 100;
  console.log(`  National: D ${natDemPct.toFixed(2)}% R ${natGopPct.toFixed(2)}% (${natTotal.toLocaleString()} votes)`);
  ...
  for (const st of states) {
    ...
    for (const r of stRows) {
      const dRatio = r.perDem > 0 ? r.perDem / natDemPct : 0;
      const rRatio = r.perGop > 0 ? r.perGop / natGopPct : 0;
