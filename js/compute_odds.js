...
function buildGBSeries() {
  ...
  for (let day = new Date(t0); day <= t1; day.setDate(day.getDate() + 1)) {
    ...
    const lo = Math.max(0, hi - GB_WINDOW_POLLS);
    const cnt = hi - lo;
    if (cnt <= 0) continue;
    series.push({
      date: fmtDate(day),
...
// ═══════════════════════════════════════════════════════
function simHouseDay(gbD, gbR, ratioD, ratioR, seatTotal, majorityLine) {
  ...
  return { pDem: demWins / MC_SIMS, expDem: seatSum / MC_SIMS };
}

// ═══════════════════════════════════════════════════════
//  STATE MC SIMULATION (from state_odds_worker.js)
// ═══════════════════════════════════════════════════════
function simStateDay(gbD, gbR, ratioD, ratioR, pollD0, pollR0, pollDDay, pollRDay,
                     nStates, dayIdx, indD, indR, wGb, wPoll, wInd,
                     baseD, baseR, total, controlLine, tieIsDem, tieSeat) {
  ...
  for (let i = 0; i < upSeats; i++) {
    ...
    if (isFinite(pDi) && isFinite(pRi)) pollPair = normalizePair(pDi, pRi);
  ...
  return { pDem: demControl / MC_SIMS, expDem: sumDSeats / MC_SIMS };
}
...
function saveOdds(file, results) {
  const obj = {
...
function main() {
  ...
  // Build state poll matrices for full date range
  ...
  // ─── HOUSE ───
  {
    ...
    if (newDays.length === 0) {
      ...
      for (let ni = 0; ni < newDays.length; ni++) {
        const i = newDays[ni];
        const r = simHouseDay(allGbD[i], allGbR[i], houseArr.d, houseArr.r, rules.total, rules.majorityLine);
        newResults.push({ date: allDates[i], pDem: +r.pDem.toFixed(4), expDem: +r.expDem.toFixed(2) });
  ...
  // ─── SENATE ───
  {
    ...
    if (newDays.length === 0) {
      ...
      for (let ni = 0; ni < newDays.length; ni++) {
        ...
        const r = simStateDay(allGbD[i], allGbR[i],
    ...
    if (newDays.length === 0) {
      ...
      for (let ni = 0; ni < newDays.length; ni++) {
        ...
        const r = simStateDay(allGbD[i], allGbR[i],
          ...
          dayIndD, dayIndR, WEIGHTS.gb, WEIGHTS.polls, WEIGHTS.ind,
          rules.baseD, rules.baseR, rules.total, controlLine,
          true, controlLine - 1);  // tieIsDem = true for governor
        newResults.push({ date: allDates[i], pDem: +r.pDem.toFixed(4), expDem: +r.expDem.toFixed(2) });
        if ((ni + 1) % 50 === 0 || ni === newDays.length - 1) process.stdout.write(`  ${ni + 1}/${newDays.length}\r`);
      }
      console.log();
