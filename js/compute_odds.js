...
function saveOdds(file, results, latestHist) {
  const obj = {
    ...
    forecast: { electionDay: fmtDate(ELECTION_DAY), fullAllocDate: fmtDate(FULL_ALLOC_DATE), splitD: UNDECIDED_SPLIT_D, splitR: UNDECIDED_SPLIT_R },
    results,
  };
...
function main() {
  ...
  const fullSeries = [...gbSeries.map(d => ({ ...d, isForecast: false })), ...forecastExt];
